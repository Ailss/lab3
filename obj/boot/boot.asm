...
protcseg:
  ...
  call bootmain
    7c45:	e8 c6 00 00 00       	call   7d10 <bootmain>
...
readseg(uint32_t va, uint32_t count, uint32_t offset)
{
    ...
    7cd4:	57                   	push   %edi
    7cd5:	8b 7d 10             	mov    0x10(%ebp),%edi
    7cd8:	56                   	push   %esi
    7cd9:	53                   	push   %ebx
...
readseg(uint32_t va, uint32_t count, uint32_t offset)
{
	...
	va &= 0xFFFFFF;
    7ce1:	89 de                	mov    %ebx,%esi
	end_va = va + count;
	
	// round down to sector boundary
	va &= ~(SECTSIZE - 1);
    7ce3:	81 e3 00 fe ff 00    	and    $0xfffe00,%ebx
void
readseg(uint32_t va, uint32_t count, uint32_t offset)
{
	uint32_t end_va;
	...
	va &= 0xFFFFFF;
    7ce9:	81 e6 ff ff ff 00    	and    $0xffffff,%esi
	end_va = va + count;
    7cef:	03 75 0c             	add    0xc(%ebp),%esi
	offset = (offset / SECTSIZE) + 1;
	...
	while (va < end_va) {
    7cf2:	39 f3                	cmp    %esi,%ebx
    7cf4:	73 12                	jae    7d08 <readseg+0x37>
    ...
    7cf8:	53                   	push   %ebx
		va += SECTSIZE;
    7cf9:	81 c3 00 02 00 00    	add    $0x200,%ebx
	...
	while (va < end_va) {
		readsect((uint8_t*) va, offset);
    7cff:	e8 78 ff ff ff       	call   7c7c <readsect>
		va += SECTSIZE;
		offset++;
    7d04:	58                   	pop    %eax
    7d05:	5a                   	pop    %edx
    7d06:	eb ea                	jmp    7cf2 <readseg+0x21>
	}
}
    7d08:	8d 65 f4             	lea    -0xc(%ebp),%esp
    7d0b:	5b                   	pop    %ebx
    7d0c:	5e                   	pop    %esi
    7d0d:	5f                   	pop    %edi
    7d0e:	5d                   	pop    %ebp
...
bootmain(void)
{
    ...
    7d15:	6a 00                	push   $0x0
    7d17:	68 00 10 00 00       	push   $0x1000
    7d1c:	68 00 00 01 00       	push   $0x10000
    7d21:	e8 ab ff ff ff       	call   7cd1 <readseg>

	// is this a valid ELF?
	if (ELFHDR->e_magic != ELF_MAGIC)
    7d26:	83 c4 0c             	add    $0xc,%esp
    7d29:	81 3d 00 00 01 00 7f 	cmpl   $0x464c457f,0x10000
    7d30:	45 4c 46 
    7d33:	75 3e                	jne    7d73 <bootmain+0x63>
		goto bad;

	// load each program segment (ignores ph flags)
	ph = (struct Proghdr *) ((uint8_t *) ELFHDR + ELFHDR->e_phoff);
    7d35:	a1 1c 00 01 00       	mov    0x1001c,%eax
    7d3a:	8d 98 00 00 01 00    	lea    0x10000(%eax),%ebx
    ...
    7d4f:	73 16                	jae    7d67 <bootmain+0x57>
    ...
    7d54:	83 c3 20             	add    $0x20,%ebx
		readseg(ph->p_va, ph->p_memsz, ph->p_offset);
    7d57:	ff 73 f4             	pushl  -0xc(%ebx)
    7d5a:	ff 73 e8             	pushl  -0x18(%ebx)
    7d5d:	e8 6f ff ff ff       	call   7cd1 <readseg>
		goto bad;
    ...
    7d65:	eb e6                	jmp    7d4d <bootmain+0x3d>
		readseg(ph->p_va, ph->p_memsz, ph->p_offset);
...
outw(int port, uint16_t data)
{
    ...
    7d86:	eb fe                	jmp    7d86 <bootmain+0x76>
